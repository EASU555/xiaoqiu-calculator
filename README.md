...
- 计算器页顶部可切换“原双结果”和“475 乘加”模式；乘加模式中的 `475` 为固定系数，不能误改。
- 475 乘加模式提供独立的“乘数、加数”输入，支持整数、小数和负数，结果同样保留最多 2 位小数。
...
- 布局：固定标题与页面切换 → 计算器页内模式选择 / 独立快捷计数页 → 页面状态提示。
- 视觉锚点：原模式使用两个结果面板；475 乘加模式突出固定公式和“等于”结果；计数页使用大号计数仪表。
